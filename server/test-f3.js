...
try {
  ...
  ).run(userId, TEST_EMAIL, hashedPassword, 'Persistence Test User', 'free');
  ...
  db2.prepare(
    `INSERT INTO projects (id, user_id, title, genre, description, area, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))`
  ).run(projectId, userId, 'Persistence Test Project', 'fiction', 'A project to test data persistence', 'romanziere');
